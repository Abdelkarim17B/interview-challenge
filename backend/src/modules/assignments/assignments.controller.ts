...
export class AssignmentsController {
  ...
  @ApiResponse({ status: 200, description: 'List of assignments with patient and medication details', type: [Assignment] })
  async findAll(): Promise<Assignment[]> {
    return await this.assignmentsService.findAll();
  }

  @Get('with-remaining-days')
  @ApiOperation({ summary: 'Get all assignments with calculated remaining days' })
  @ApiResponse({ status: 200, description: 'List of assignments with remaining days calculated' })
  ...
  @ApiOperation({ summary: 'Get an assignment by ID with calculated remaining days' })
  @ApiParam({ name: 'id', description: 'Assignment ID' })
  @ApiResponse({ status: 200, description: 'Assignment found with remaining days calculated' })
  @ApiResponse({ status: 404, description: 'Assignment not found' })
  async findOneWithRemainingDays(@Param('id', ParseIntPipe) id: number): Promise<Assignment & { remainingDays: number }> {
