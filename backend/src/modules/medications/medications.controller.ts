...
export class MedicationsController {
  ...
  @ApiResponse({ status: 201, description: 'Medication created successfully', type: Medication })
  @ApiResponse({ status: 400, description: 'Bad request - validation failed' })
  async create(@Body(ValidationPipe) createMedicationDto: CreateMedicationDto): Promise<Medication> {
    return await this.medicationsService.create(createMedicationDto);
  }

  @Get()
  @ApiOperation({ summary: 'Get all medications' })
  @ApiResponse({ status: 200, description: 'List of medications with their assignments', type: [Medication] })
  ...
  @ApiResponse({ status: 200, description: 'Medication found', type: Medication })
  ...
  @ApiResponse({ status: 200, description: 'Medication updated successfully', type: Medication })
  @ApiResponse({ status: 404, description: 'Medication not found' })
  @ApiResponse({ status: 400, description: 'Bad request - validation failed' })
  async update(
